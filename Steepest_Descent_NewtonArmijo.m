...
%Calculo del tamaño de paso: Newton+Condicion de Armijo
...
function [x1,fx1,iter]=Steepest_Descent_NewtonArmijo(f,x0,eps1)
  global itern
  iter = 0;
  ...
  while  norma > eps1    
    ...
    %Condicion de Armijo para comprobar la optimalidad de alpha
    c=0.5; %Valor de c1
    while f(x1+(alpha*nu))>(f(x1)+(c*alpha*(Gradiente(f,x1)).'*nu))
        alpha=0.5*alpha;
        itern = itern + 1;
    end %while
    %
    %Reasignacion del valor optimo
    x1=x0 + alpha*nu;
    fx1   = f(x1);
    x0    = x1;
    fx0   = fx1;
